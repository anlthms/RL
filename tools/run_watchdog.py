...
**GPU memory activity is the progress signal.** Not log output: a job can be
quiet for many minutes while doing real work (a long evaluation pass, a slow
checkpoint), and treating silence as failure kills healthy runs. And not SM
utilization alone: ranks parked in a stuck NCCL collective spin-wait at ~100%
"utilization" while moving nothing -- DRAM traffic separates real work from
both that and plain idleness, and keeps this tool free of any knowledge of the
application running inside the job.
...
# Memory-controller utilization at or below this percent means no real data is
# moving. Discriminates a stuck NCCL collective from work: a spin-wait kernel
# keeps the SMs "busy" polling a flag, but any actual compute streams weights
# and activations through DRAM.
G_MEM_BUSY_PERCENT = 1

# Fraction of the job's GPUs that must be working for the job to count as making
...
class Observation:
    ...
    ``working_fraction`` is the share of GPUs moving data through DRAM (real
    progress); ``spinning_fraction`` is the share that are SM-busy but
    memory-idle -- the signature of ranks parked in a stuck NCCL collective,
    which reads as ~100% "utilization" while doing nothing.
    """

    state: str
    working_fraction: float | None
    spinning_fraction: float | None
    nodes: list[str]
    ...
    def is_stalled(self) -> bool:
        """Stalled means measured and (nearly) no GPU is doing real work.

        Covers both failure shapes: all-idle and all-spinning. An unavailable
        sample is not a vote: a probe that failed to run is no evidence that
        the GPUs are doing nothing, and treating it as such would let a
        transient ``srun`` failure kill a healthy job.
        """
        return (
            self.working_fraction is not None
            and self.working_fraction < G_BUSY_FRACTION
        )
...
def activity_fractions(
    job_id: str, nodes: list[str]
) -> tuple[float | None, float | None]:
    """(working, spinning) fractions of the job's GPUs, or (None, None) if unknown.

    Working = memory controller active (real data movement). Spinning = SMs
    busy but memory idle, i.e. a spin-wait such as a stuck NCCL collective.
    On GPUs whose memory reading is N/A, SM utilization alone decides
    (the pre-spin-detection behavior).
    """
    if not nodes:
        return None, None
    code, out = _in_job(
    ...
    if code != 0:
        return None, None
    readings = re.findall(
        r"^\s*(\d+)\s*,\s*(\d+|\[?N/A\]?)\s*$", out, flags=re.MULTILINE
    )
    if not readings:
        return None, None
    working = spinning = 0
    for sm_str, mem_str in readings:
        sm_busy = int(sm_str) > G_BUSY_PERCENT
        if "N/A" in mem_str:
            working += sm_busy
        elif int(mem_str) > G_MEM_BUSY_PERCENT:
            working += 1
        elif sm_busy:
            spinning += 1
    return working / len(readings), spinning / len(readings)
...
def observe(job_id: str) -> Observation:
    ...
    if any(state.startswith(term) for term in G_TERMINAL_STATES) or state == "UNKNOWN":
        return Observation(state, None, None, [])
    nodes = job_nodes(job_id)
    working, spinning = activity_fractions(job_id, nodes)
    return Observation(state, working, spinning, nodes)
    ...
    (out_dir / "summary.txt").write_text(
        "\n".join(
            [
                ...
                f"gpu_working_fraction: {observation.working_fraction}",
                f"gpu_spinning_fraction: {observation.spinning_fraction}",
                f"stall_threshold: <{G_BUSY_FRACTION} of GPUs with memory util above {G_MEM_BUSY_PERCENT}%",
    ...
    *,
    ...
    while True:
        ...
        if in_grace:
            ...
            consecutive_idle = 0
        elif observation.is_stalled:
            consecutive_idle += 1
        elif observation.working_fraction is not None:
            consecutive_idle = 0
        ...
        working = (
            "unknown"
            if observation.working_fraction is None
            else f"{observation.working_fraction:.0%}"
        )
        spin_note = (
            f" gpu_spinning={observation.spinning_fraction:.0%}"
            if observation.spinning_fraction
            else ""
        )
        grace_note = f" grace({elapsed:.0f}/{grace_minutes:.0f}m)" if in_grace else ""
        print(
            f"[{stamp}] {job_id} state={observation.state} gpu_working={working}"
            f"{spin_note} stall_polls={consecutive_idle}/{idle_polls}{grace_note}",
            flush=True,
        )
        ...
        if consecutive_idle >= idle_polls:
            ...
            shape = (
                "busy-spinning (stuck collective?)"
                if (observation.spinning_fraction or 0) >= G_BUSY_FRACTION
                else "idle"
            )
            print(
                f"HANG CONFIRMED: GPUs {shape} for {minutes:.0f} min. "
