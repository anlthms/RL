...
checkpoint — so treating silence as failure kills healthy runs. Nor is SM
utilization alone trustworthy: ranks parked in a stuck NCCL collective
spin-wait at ~100% "utilization" while moving nothing. DRAM traffic
(`utilization.memory`) separates real work from both plain idleness and that
busy-spinning, which also keeps the tool free of any knowledge of the
application inside the job.

Each GPU is classified per poll: **working** (memory util > 1%), **spinning**
(SM util > 5% but memory idle — the stuck-collective signature), or **idle**.
...
2. Fewer than 5% of its GPUs are working.
3. That has been true for `--idle-polls` consecutive polls.
4. The job is past `--grace-minutes`.

The trip message says which shape was seen — idle GPUs, or busy-spinning ones
that indicate a stuck collective.
...
evidence the GPUs are idle, so a transient `srun` failure can neither trip
...
| File | Contents |
|---|---|
| `summary.txt` | job state, GPU working/spinning fractions, nodes, capture time |
